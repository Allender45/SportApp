...
export default function WorkoutDetailScreen({route}: Props) {
    ...
    const incrementSets = (id: string, max: number) => {
        setCompletedSets(prev => ({...prev, [id]: Math.min((prev[id] ?? 0) + 1, max)}));
    };

    const decrementSets = (id: string) => {
        setCompletedSets(prev => ({...prev, [id]: Math.max((prev[id] ?? 0) - 1, 0)}));
    };
...
const styles = StyleSheet.create({
    ...
    screen: {flex: 1},
    footer: {
        backgroundColor: 'rgba(20, 23, 28, 0.97)',
        borderTopWidth: 1,
    ...
    setsTrackerRow: {flexDirection: 'row', alignItems: 'center', marginBottom: 6, marginTop: 2},
    setsTrackerBtn: {
        width: 22,
        height: 22,
        borderRadius: 11,
        backgroundColor: 'rgba(143,149,158,0.15)',
        alignItems: 'center',
        justifyContent: 'center'
    },
    setsTrackerBtnText: {color: '#BDC4CE', fontSize: 16, fontWeight: 'bold', lineHeight: 20},
    setsSegments: {flexDirection: 'row', marginHorizontal: 8, gap: 3},
    setsSegment: {width: 5, height: 18, borderRadius: 3},
    setsSegmentDone: {backgroundColor: '#4A8B4A'},
    setsSegmentEmpty: {backgroundColor: 'rgba(143,149,158,0.12)', borderWidth: 1, borderColor: '#3A3F47'},
    cardImageContainer: {position: 'absolute', right: 0, top: 0, bottom: 0, width: 160},
